...
import { SButtom, SGeolocation, SHr, SInput, SMapView, SNavigation, SNotification, SPage, SText, STheme, SView, SIcon, SPopup } from 'servisofts-component';
import PopupAutoCompleteDireccion from '../../../../../Pages/direccion/Components/PopupAutoCompleteDireccion';
...
export default class select extends Component {
    ...
    renderMenu() {
        ...
        return <SView col={"xs-11"} height={140} backgroundColor='#018992AA' style={{
            position: "absolute",
            bottom: 8,
            borderRadius: 8,
            padding: 8,
            ...
            <SInput
                ...
                // backgroundColor={STheme.color.white}
                style={{
                    // backgroundColor: STheme.color.WHI,
                    // color: STheme.color.text,
                }}
                placeholder={"Dirección, Ejemplo: Av Banzer, Santa Cruz"} />
            <SText height={25} font='LondonMM' color={STheme.color.white}>{this.state.error}</SText>
            {/* <SHr height={10} /> */}
            <SButtom type='secondary' onPress={() => {
                // this.input.submit()
                this.state.direccion = this.input.getValue();
                if (!this.state.direccion) {
                    this.setState({ error: "Debes registrar una descripción de dirección."});
                    return;
                }
                this.setState({ loading: true, error: "" })
                console.log(this.state)
                if (this.onSelect) this.onSelect(this.state)
            }}><SText color={STheme.color.text}  font='LondonTwo' >REGISTRAR</SText>
            </SButtom>
        </SView>
    }
