...
export default (state: any, action: DataProps) => {
    ...
    if (action.component != "carrito") return state;
...
const TypesSwitch = (state: any, action: DataProps) => {
    switch (action.type) {
        ...
        case "eliminar": return eliminar(state, action);
...
const registro = (state: any, action: DataProps) => {
    ...
    if (state.data[action.data.key]) {
        state.data[action.data.key].cantidad += action.data.cantidad;
        return;
    }
    state.data[action.data.key] = action.data;
}
const eliminar = (state: any, action: DataProps) => {
    if (action.estado != "exito") return;
    if (!state.data) return;
