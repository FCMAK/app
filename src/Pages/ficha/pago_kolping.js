...
class Pago_kolping extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getTotal() {
        let total = 0;
        const detalle = this.state?.data?.solApp?.solSer;
        if (detalle) {
            detalle.map((a) => {
                total += a.prePro
            })
                ...
                <Container loading={!this.state.data}>
                    <SView col={"xs-12"} center>
                        <SView col={"xs-12"} center >
                            ...
                            <SHr height={30} />
                            <SText center font={"LondonMM"} fontSize={20}>Su ficha ha sido reservada correctamente. A continuación puede descargar su orden de atencion y acceder a la factura a través de la plataforma de impuestos SIAT</SText>
                            <SHr height={20} />
                            <SView col={"xs-12"} card padding={20} row>
                                <SView col={"xs-6"} row>
                                    <SText color={STheme.color.darkGray} font='LondonBetween'>Nro. de transacción: </SText>
                                    <SText bold font='LondonBetween'>{nroGrl}</SText>
                                </SView>
                                <SView col={"xs-6"} flex style={{ alignItems: 'flex-end' }}>
                                    <SView row >
                                        <SText color={STheme.color.darkGray} font='LondonBetween'>Fecha: </SText>
                                        <SText font='LondonBetween' bold>{new SDate(fecReg, "yyyy-MM-ddThh:mm:ss").toString("dd-MM-yyyy hh:mm")}</SText>
                                    </SView>
                                </SView>
                                <SView col={"xs-12"} />
                                <SHr />
                                <SView col={"xs-6"} row>
                                    ...
                                    <SText style={{ textTransform: "uppercase" }} bold font='LondonBetween'>{solApp?.datNom}</SText>
                                </SView>
                                <SHr height={15} />
                                ...
                                <SText style={{ textTransform: "uppercase" }} bold font='LondonBetween'>{solApp?.solSer?.[0].codTur}{solApp?.solSer?.[0].comTur} | {solApp?.solSer?.[0].horTur} | {new SDate(solApp?.solSer?.[0].fecSol, "yyyy-MM-ddThh:mm:ss").toString("DAY dd MONTH del yyyy")}</SText>
                                <SHr />
                                ...
                                <SView row col={"xs-12"}>
                                    ...
                                    <SText style={{ textTransform: "uppercase" }} bold font='LondonBetween'>{solApp?.solSer?.[0].nomMed} </SText>
                                    <SText center fontSize={12} color={STheme.color.info} style={{ textTransform: "uppercase" }} bold font='LondonBetween'>({solApp?.solSer?.[0].nomEsp}) </SText>
                                </SView>
                                <SHr height={15} />
                                <SText color={STheme.color.darkGray} fontSize={16} font='LondonBetween'>Detalle Compra: </SText>
                                <SHr h={1} color={STheme.color.lightGray} />
                                <SView row col={"xs-12"} >
                                    <SHr height={10} />
                                    {solApp?.solSer.map((item, key) => {
                                        return <SView col={"xs-12"} row key={key}>
                                            <SText col={"xs-6"} bold fontSize={14} font='LondonBetween'>{item?.codPro} {item?.nomPro}</SText>
                                            <SView col={"xs-6"} row>
                                                <SView flex />
                                                <SText style={{ alignItems: "flex-end" }} bold fontSize={14} font='LondonBetween'>Bs. </SText>
                                                <SText style={{ textAlign: "right", width: 50 }} bold fontSize={14} font='LondonBetween'>{item?.prePro}</SText>
                                            </SView>
                                        </SView>
                                    })}
                                    <SHr height={10} />
                                ...
                                <SView col={"xs-12"} style={{ alignItems: "flex-end" }} >
                                    <SText fontSize={20} bold font={"LondonBetween"} color={STheme.color.text}>Bs. {SMath.formatMoney(this.getTotal())}</SText>
                                </SView>
                                <SHr height={10} />
                            </SView>
                            <SHr height={20} />
                            <SView col={"xs-12"} row center >
                                <SView width={150} height={50} style={{
                                    ...
                                    onPress={() => {
                                        if (!this.state?.data?.odaImg) return;

                                        // Esta viene en png
                                        SShared.sharedB64(`data:image/png;base64,${this.state?.data?.odaImg}`, {
                                ...
                                <SView width={150} height={50} style={{
                                    ...
                                    onPress={() => {
                                        Linking.openURL(this.state?.data?.facUrl)
                                    }}>
                                    <SHr height={10} />
...
export default connect(initStates)(Pago_kolping);