...
export default class ListaMedicos extends Component {
    ...
    renderMedicos(data){
        if (data.length === 0) return <NoData mensaje={"No tenemos médicos disponibles en esta especialidad."} />
        return  <FlatList
        style={{ width: "100%" }}
        data={this.handleFilter((data))}
        ItemSeparatorComponent={() => <SHr />}
        renderItem={({ item }) => <MedicoItem medico={item} onPress={() => {
            SNavigation.navigate("/info/horarios", { nrosuc: this.nrosuc, codmed: item.CodMed, fecha: this.state.fecha })
        }} />}
    />
    }
