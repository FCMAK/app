...
export default class SMD extends Component<SMDPropsType> {
    ...
    buildImageLink({ altText, imgUrl, linkUrl, style }: any) {
        var label = altText;
        // let regex = new RegExp(/\[(.*)\]\((.*)\)/g);

        // let match: any = regex.exec(word);
        // console.log(word, match)
        return <SView onPress={() => { Linking.openURL(linkUrl) }} >
            <ScaledImage alt={altText}
        ...
        return <Text onPress={() => { Linking.openURL(linkUrl) }} style={{ ...style, color: this.props.colorLink }}>{imgUrl}</Text>
    }
    ...
    buildLine(line: string, style: TextStyle = {}) {
        ...
        let regImgLink = /\[\!\[(.*?)\]\((.*?)\)\]\((.*?)\)/;
        while ((match = regImgLink.exec(line)) !== null) {
            let altText = match[1];
            let imgUrl = match[2];
            let linkUrl = match[3];

            let nstyle = { ...style };
            ELEMENTS.push(this.buildImageLink({ altText, imgUrl, linkUrl, style: nstyle }));
            line = line.substring(0, match.index) + `\${${ELEMENTS.length - 1}}` + line.substring(match.index + match[0].length);
        }
        reg = new RegExp(/\[(.*?)\]\((.*?)\)/);
        while ((match = reg.exec(line)) !== null) {
            let text = match[0];
